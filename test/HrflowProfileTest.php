...
final class HrflowProfileTest extends TestCase {
  ...
  private function useApiFuncWithValidProfile($profile_ids, $profileFunc) {
    foreach ($profile_ids as $profile_idPair) {
      ...
      $isResponseExp = $err instanceof HrflowApiResponseException;
  ...
  private function getSomeFilterIdsPair($api, $n_ids = 5) {
    ...
    foreach ($filters as $filter) {
      ...
      $job_ids[] = array('id' => $filter['job_id'],
        'reference' => $filter['filter_reference']);
      if (count($job_ids) > $n_ids) {
        break;
      }
    }
    return $job_ids;
  ...
  private function getSomeProfileIdsPair($api) {
    ...
    $args = array(
      HrflowField::SOURCE_IDS => $source_ids,
  ...
  public function testGetProfilesFullArgs(): void {
      ...
      $args = array(
        ...
        HrflowField::DATE_START => $start->getTimestamp(),
        'date_end' => $end,
        HrflowField::SENIORITY => $seniority,
        HrflowField::job_id => $job_id,
        HrflowField::STAGE => $stage,
        'page' => $page,
        'limit' => $limit,
        'sort_by' => $sort_by,
        'order_by' => $order_by
  ...
  public function testGetProfilesFullArgs_filterID(): void {
      ...
      $seniority = "senior";
      $source_ids = $this->getSomeNotSharedSourceIds($api);
      $job_id = $this->getSomeFilterIdsPair($api)[0]['id'];
      $stage = "NEW";
      $page = 2;
      $limit = 10;
      $sort_by = "ranking";
  ...
  public function testGetProfilesFullArgs_filterReference(): void {
      ...
      $args = array(
        ...
        'date_end' => $end,
        HrflowField::SENIORITY => $seniority,
        HrflowField::FILTER_REFERENCE => $filter_reference,
        HrflowField::STAGE => $stage,
        'page' => $page,
        'limit' => $limit,
        'sort_by' => $sort_by,
  ...
  public function testGetProfilesErrDate(): void {
      ...
      $resp = TestHelper::useApiFuncWithExpectedErr($this, $getProfiles, 'HrflowApiArgumentException');
  }
  ...
  public function testGetProfileWithInvalidProfileSourceId(): void {
      ...
      $getProfile = function () use ($api, $profile_id, $source_id)
        {  return $api->profile->get(new ProfileID($profile_id), $source_id); };
      $resp = TestHelper::useApiFuncWithExpectedErr($this, $getProfile, 'HrflowApiResponseException');
  }
  ...
  public function testGetJobsWithInvalidScoringSourceId(): void {
      ...
      $resp = TestHelper::useApiFuncWithExpectedErr($this, $getScoring, 'HrflowApiResponseException');
  }
  ...
  public function testUpdateStage(): void {
    ...
    $updateProfile = function () use ($api, $profile_id, $source_id, $job_id, $stage)
      {  return $api->profile->stage->set(new ProfileID($profile_id), $source_id, new FilterID($job_id), $stage); };
  ...
  public function testUpdateStage_reference(): void {
    ...
    public function testUpdateRating(): void {
      ...
      $updateProfile = function () use ($api, $profile_id, $source_id, $job_id, $rating)
        {  return $api->profile->rating->set(new ProfileID($profile_id), $source_id, new FilterID($job_id), $rating); };
      $resp = TestHelper::useApiFuncWithReportedErr($this, $updateProfile);
      TestHelper::assertArrayHasKeys($this, $resp, $refKeys);
  ...
  public function testJsonAdd_bad_metadata(): void {
        ...
        TestHelper::useApiFuncWithExpectedErr($this, $addData, 'HrflowApiArgumentException');
  }
